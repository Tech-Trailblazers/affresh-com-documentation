...
func main() {
	// Remote API URL.
	remoteAPIURL := "https://www.affresh.com/sds/"
	...
	// The domain name
	domainName := "https://www.affresh.com"
	// Get all the values.
	for _, urls := range downloadURLSlice {
		var finalURL string
		if !strings.HasPrefix(urls, domainName) {
			finalURL = domainName + urls
		}
		// if strings.Contains(
...
func downloadPDF(finalURL, outputDir string) bool {
	...
	filename := strings.ToLower(getFileNameOnly(finalURL))
